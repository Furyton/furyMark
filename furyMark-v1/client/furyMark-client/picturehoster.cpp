...
void PictureHoster::sendPost()
{
  ...
  QString strFilePath = QFileDialog::getOpenFileName(0, QString::fromLocal8Bit("Select a picture"), "./", tr("Image files(*.bmp *.jpg *.pbm *.pgm *.png *.ppm *.xbm *.xpm *.jpeg);;All files (*.*)"));
    ...
    if(reply->error() == QNetworkReply::NoError)
    {
      ...
      //我们只需要上传后的url，所以值提取了url，如果你需要其他内容，请提取方法类似
        if(rootObj.contains("data"))
        {
           ...
           emit urlSignal(subObj["url"].toString());
        }
    }
    else
    {
